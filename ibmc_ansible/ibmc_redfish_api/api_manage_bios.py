...
def get_bios_setting_result(ibmc, bios_info):
    """
    Function:
    ...
    for key, value in bios_info.items():
        if value != current_bios.get(key):
            waite_bios_set[key] = value

    # Obtain the verification result.
    if waite_bios_set:
        log_info = " Parameters are waiting to take effect: %s. " \
                   % waite_bios_set
        set_result(ibmc.log_info, log_info, False, ret)
    else:
